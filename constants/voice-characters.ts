import type { VoiceCharacter } from '../packages/shared/types';

export const VOICE_CHARACTERS: VoiceCharacter[] = [
  {
    id: 'narrator',
    name: 'Storyteller',
    nameKey: 'voiceStoryteller',
    emoji: '\u{1F4D6}',
    description: 'Warm, captivating storyteller',
    descKey: 'voiceStorytellerDesc',
    pitch: 1.0,
    rate: 1.0,
    gradient: ['#FF6B6B', '#FF8E53'],
    isPremium: false,
    openaiVoice: 'nova',
    elevenLabsVoiceId: '21m00Tcm4TlvDq8ikWAM',
  },
  {
    id: 'fairy',
    name: 'Fairy Sparkle',
    nameKey: 'voiceFairy',
    emoji: '\u{1F9DA}',
    description: 'Playful, bright and warm',
    descKey: 'voiceFairyDesc',
    pitch: 1.1,
    rate: 0.95,
    gradient: ['#C77DFF', '#E8A0FF'],
    isPremium: false,
    openaiVoice: 'shimmer',
    elevenLabsVoiceId: 'MF3mGyEYCl7XYWbV9V6O',
  },
  {
    id: 'wizard',
    name: 'Old Wizard',
    nameKey: 'voiceWizard',
    emoji: '\u{1F9D9}',
    description: 'Wise, mature and balanced',
    descKey: 'voiceWizardDesc',
    pitch: 0.9,
    rate: 0.9,
    gradient: ['#1A1A6E', '#4A4AE0'],
    isPremium: true,
    openaiVoice: 'onyx',
    elevenLabsVoiceId: 'pNInz6obpgDQGcFmaJgB',
  },
  {
    id: 'robot',
    name: 'Beep Bot',
    nameKey: 'voiceRobot',
    emoji: '\u{1F916}',
    description: 'Energetic and fun',
    descKey: 'voiceRobotDesc',
    pitch: 0.95,
    rate: 1.05,
    gradient: ['#00B4D8', '#48CAE4'],
    isPremium: true,
    openaiVoice: 'echo',
    elevenLabsVoiceId: 'VR6AewLTigWG4xSOukaG',
  },
  {
    id: 'pirate',
    name: 'Captain Arr',
    nameKey: 'voicePirate',
    emoji: '\u{1F3F4}\u{200D}\u{2620}\u{FE0F}',
    description: 'Deep, confident adventurer',
    descKey: 'voicePirateDesc',
    pitch: 0.9,
    rate: 0.95,
    gradient: ['#D4380D', '#FF7A45'],
    isPremium: true,
    openaiVoice: 'fable',
    elevenLabsVoiceId: 'TxGEqnHWrfWFTfGW9XjX',
  },
  {
    id: 'princess',
    name: 'Princess Rose',
    nameKey: 'voicePrincess',
    emoji: '\u{1F478}',
    description: 'Sweet, gentle and clear',
    descKey: 'voicePrincessDesc',
    pitch: 1.1,
    rate: 0.95,
    gradient: ['#FF69B4', '#FFB6C1'],
    isPremium: true,
    openaiVoice: 'shimmer',
    elevenLabsVoiceId: 'MF3mGyEYCl7XYWbV9V6O',
  },
  {
    id: 'dragon',
    name: 'Drago',
    nameKey: 'voiceDragon',
    emoji: '\u{1F432}',
    description: 'Deep, resonant and comforting',
    descKey: 'voiceDragonDesc',
    pitch: 0.85,
    rate: 1.0,
    gradient: ['#52B788', '#95D5B2'],
    isPremium: true,
    openaiVoice: 'onyx',
    elevenLabsVoiceId: 'pNInz6obpgDQGcFmaJgB',
  },
  {
    id: 'alien',
    name: 'Zibzob',
    nameKey: 'voiceAlien',
    emoji: '\u{1F47D}',
    description: 'Quirky, husky trickster',
    descKey: 'voiceAlienDesc',
    pitch: 1.15,
    rate: 1.05,
    gradient: ['#A8E6CF', '#DCEDC1'],
    isPremium: true,
    openaiVoice: 'alloy',
    elevenLabsVoiceId: 'yoZ06aMxZJJ28mfd3POQ',
  },
];

export function getVoiceCharacterById(id: string): VoiceCharacter | undefined {
  return VOICE_CHARACTERS.find(v => v.id === id);
}
